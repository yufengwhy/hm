...
def pmap_multi(pickleable_fn, data, n_jobs=None, verbose=1, desc=None, backend=None):
    if n_jobs is None:
        n_jobs = cpu_count() - 1
    jobs = [delayed(pickleable_fn)(*d) for i, d in tqdm(enumerate(data), desc=desc)]
    results = Parallel(n_jobs=n_jobs, verbose=verbose, timeout=None, backend=backend)(jobs)
    return results
...
def my_affinity_criterion(y_pred, y, mask, mode, k_pos, k_neg, decoy_gap=1.0):
    affinity_loss = torch.zeros(y_pred.shape).to(y_pred.device)
    affinity_loss[mask] = (((y_pred - y)**2)[mask]) * k_pos
    if mode=='mse':
        affinity_loss[~mask] = ( ((y_pred - (y - decoy_gap)).relu())**2 * k_neg )[~mask]
    elif mode=='bpr':
        affinity_loss[~mask] = ( - (y - y_pred).sigmoid().log() * k_neg )[~mask]
    elif mode=='bpr_hinge':
        affinity_loss[~mask] = ( (y_pred - (y - decoy_gap)).relu().sigmoid().log() * k_neg )[~mask]
...
def evaulate_with_affinity(data_loader, model, criterion, affinity_criterion, relative_k, device, pred_dis=False, info=None, saveFileName=None, use_y_mask=False, skip_y_metrics_evaluation=False, distributed=False):
    y_list = []
    y_pred_list = []
    affinity_list = []
    affinity_pred_list = []
    ...
    for data in tqdm(data_loader):
        ...
        data = data.to(device)
        if distributed:
            # refer to https://discuss.pytorch.org/t/distributeddataparallel-barrier-doesnt-work-as-expected-during-evaluation/99867/11
            y_pred, affinity_pred = model.module(data) 
        else:
...
def read_write(mode, fn, item=None):
    if mode=='w':
